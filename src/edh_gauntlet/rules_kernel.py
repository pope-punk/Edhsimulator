...
class RulesKernel(RoomRules,FaceRules,WalkerRules,RuleEffects,ResolutionCastingRules,SpellCopyRules,CopyRules,ManaRules,GuardRules,PhasingRules,CounterRules,LibraryRules,DepartureRules,CombatRules,TurnRules,CastingRules,AttachmentRules):
    ...
    def definition(self,obj):
        ...
        if obj.zone==Zone.BATTLEFIELD and hasattr(self,'temporary_effects'):
            ...
            if view is None and obj.ref in self.last_known:view=self.last_known[obj.ref][1]
            if view is not None and view.abilities_removed:
                return replace(program,abilities=(),activated=(),continuous=(),entry_modifiers=(),entry_counters=(),
    ...
    def _player_event(self,kind,player,**data):
        if kind=='card_drawn':
            if self.draw_count_turn!=self.state.turn_number:
                self.draw_counts={};self.draw_count_turn=self.state.turn_number
            self.draw_counts[player]=self.draw_counts.get(player,0)+1
            data={**data,'draw_ordinal':self.draw_counts[player]}
        self._event(kind,player=player,**data)
        for source in self.state.objects(Zone.BATTLEFIELD):
            if source.phased:continue
            for ability in self._trigger_abilities(source,kind):
                pattern=ability.event
                if isinstance(pattern,DrawEventPattern) and pattern.occurrence!=self.draw_counts.get(player,0):continue
    ...
    def _matches(self,pattern,source,*,event=None,step=None,views=None):
        ...
        if isinstance(pattern,ZoneEventPattern):
            if pattern.cause is not None and event.cause!=pattern.cause:return False
            if pattern.destination_owned and event.after.owner!=source.controller:return False
        ...
        if isinstance(pattern,ZoneEventPattern) and not set(pattern.subtypes)<=view.subtypes:return False
        ...
        for event in events:
            ...
            for sources,lookback in ((before,True),(after,False)):
                for source in sources:
                    if source.phased:continue
                    for ability in self._trigger_abilities(source,'zone_changed',before_views if lookback else after_views):
                        ...
                        if self._matches(ability.event,source,event=event,views=before_views if lookback else after_views):
                            ...
                            if isinstance(ability.event,ZoneEventPattern):
                                bindings['event_successor']=[event.after.ref.to_json()] if event.after.zone in {Zone.BATTLEFIELD,Zone.STACK,Zone.GRAVEYARD,Zone.EXILE,Zone.COMMAND} else []
    ...
    def _condition_holds(self,condition,source,objects=None,views=None,life_totals=None,live_players=None,life_lost_totals=None):
        ...
        if isinstance(condition,LifeGainedCondition):return self.state.life_gained_this_turn(source.controller)>=condition.minimum
    ...
    def _trigger(self,source,ability,bindings=None,*,condition_objects=None,condition_views=None,condition_life_totals=None,condition_live_players=None,condition_life_lost=None,values=None):
        ...
        if isinstance(ability,ChapterAbility):values['saga_chapter']=ability.chapter
    ...
    def _entry_effect_candidates(self, proposal, entry_view):
        ...
        result=list(self._intrinsic_entry_counters(proposal,entering,view))
        for source_ref,effect_id in view.riot:
            key='riot:'+source_ref.card_id+'@'+str(source_ref.incarnation)+':'+effect_id
            if key not in proposal.used:
                result.append(ReplacementCandidate(key,'Riot: counter or haste','riot',3,entering.controller))
    ...
    def _resolve_zone_proposal(self, proposal, frame, key, *, reserved_life=None):
        ...
        while True:
            ...
            available = candidates(self.state, self.definitions, proposal, types,applicable,
                frozenset(ref for ref,view in self.characteristics().items() if view.abilities_removed))
            ...
            if candidate.kind == 'commander':
                ...
                note=(candidate.program.note_id,self.state.life(actor)-reserved)
            ...
            elif candidate.kind=='riot':
                chosen=self._choose(step_key+':riot',proposal.controller,'riot',
                    'Have this permanent enter with a +1/+1 counter, or gain haste.',
                    (Option('counter','A +1/+1 counter'),Option('haste','Haste')),1,1)
                if chosen[0].key=='counter':
                    counts=dict(proposal.counters);counts['+1/+1']=counts.get('+1/+1',0)+1
                    counters=tuple(sorted(counts.items()))
                else:proposal=replace(proposal,riot_haste=True)
    ...
    def _move(self, refs, destination, frame, key, *, cause='effect', entry_flags=(), controller_mode='effect', detaches=(), counter_pairs=(),payment=None,entry_tapped=False,creates=(),placements=None,entry_counters=(),destruction_refs=(),defer_library_tops=False,entry_subtypes=(),back_face=None,causes=None):
        ...
        for player in players:
            ...
            relevant = [move for move in own if move.destination == Zone.BATTLEFIELD
                and self.definitions[move.copied_definition or (get_source(move.source).definition+':back' if move.back_face else get_source(move.source).definition)].continuous]
            if len(relevant) > 1:
                options = tuple(Option(str(i), self.definitions[move.copied_definition or (get_source(move.source).definition+':back' if move.back_face else get_source(move.source).definition)].name,
        ...
        events = self.state.move(ordered_moves, cause, regenerations=tuple(regenerated.values()),detaches=detaches, counter_pairs=counter_pairs,payment=payment,creates=tuple(created[m.source] for m in ordered_moves if m.source in created),
                                 entry_life=tuple((actor,entry_life[actor]) for actor in players if entry_life.get(actor)),causes={m.source:causes[m.source] for m in ordered_moves if causes and m.source in causes})
        ...
        for proposal in resolved.values():
            ...
            obj=entered[proposal.before.ref]
            self.object_notes[self._attachment_key(obj.ref)]={'ref':obj.ref.to_json(),'values':dict(proposal.notes)}
            self._event('entry_life_noted',source=obj.ref.to_json(),notes=dict(proposal.notes))
    ...
    def _quantity(self,value,frame):
        ...
        if isinstance(value,CountDistinctNames):return len({self.definition(obj).name for obj in self._query(value.selector,frame)})
    ...
    def _zone_operation(self,effect,frame,key):
        ...
        if isinstance(effect,Move):
            ...
            events=self._move(refs,effect.destination,frame,key,entry_flags=frame['entry_flags'] if effect.subject=='source' else (),controller_mode=effect.controller,entry_tapped=effect.tapped,entry_counters=effect.counters,defer_library_tops=effect.library_position is not None,entry_subtypes=effect.added_subtypes if isinstance(effect,MoveWithSubtypes) else (),back_face=effect.back_face if isinstance(effect,MoveFace) else None)
            if effect.library_position is not None:
            ...
            for ref in self._refs(frame,effect.subject):
                ...
                if obj.zone==Zone.STACK and not self._spell_uncounterable(ref):refs.append(ref)
    ...
    def _execute(self,frame,task):
        ...
        if isinstance(effect,PayMana):
            ...
            self._insert(frame,effect.effects if window['paid'] else effect.otherwise)
            self.mana_payment=None
        elif isinstance(effect,PayLifeOrSacrifice):
            players=self._players(frame,effect.players)
            if not players:self._insert(frame,effect.otherwise);return
            if len(players)!=1:raise UnsupportedRule('Alternate payment requires one payer')
            payer=players[0];pay_frame={**frame,'controller':payer}
            options=(Option('decline','Allow the unpaid effect'),)
            if self.state.life(payer)>=effect.life:options+=(Option('life','Pay '+str(effect.life)+' life'),)
            options+=tuple(Option('sacrifice:'+o.key,'Sacrifice '+o.label,ref=o.ref)
                           for o in self._options(self._query(effect.selector,pay_frame)))
            selected=self._choose(key+':payment',payer,'life_or_sacrifice',
                'Pay life, sacrifice one matching permanent, or decline.',options,1,1)[0]
            if selected.key=='life':
                self.state.move((),'resolution_payment',payment=ResourcePayment(payer,life=effect.life))
                self._event('life_lost',player=payer,amount=effect.life,cause='resolution_payment')
            elif selected.ref is not None:
                self._move((selected.ref,),Zone.GRAVEYARD,pay_frame,key+':sacrifice',cause='sacrifice')
            self._insert(frame,effect.otherwise if selected.key=='decline' else effect.effects)
        elif isinstance(effect,IfQuantityAtLeast):
            self._insert(frame,effect.effects if self._quantity(effect.value,frame)>=effect.minimum else effect.otherwise)
        elif isinstance(effect,NoteLife):
            row=self.object_notes.get(self._attachment_key(source.ref))
            if row is None or effect.note_id not in row['values']:
                raise UnsupportedRule('Missing exact-incarnation life note')
            if isinstance(effect,CompareLifeNote):
                self._insert(frame,effect.effects if self.state.life(controller)>=row['values'][effect.note_id] else effect.otherwise)
            else:
                row['values'][effect.note_id]=self.state.life(controller)
                self._event('life_noted',source=source.ref.to_json(),note=effect.note_id,value=row['values'][effect.note_id],controller=controller)
        elif isinstance(effect,IfPaidCostSubtype):
            try:paid=frame['values']['paid_cost_subtypes'][effect.cost_id]
            except KeyError as exc:raise UnsupportedRule('Missing paid-cost subtype observation') from exc
            matches=bool(set(paid)&expanded_subtypes(effect.subtypes,effect.sets))
            self._insert(frame,effect.effects if matches else effect.otherwise)
        elif isinstance(effect,UntilEndOfTurn):
            refs=[]
            for ref in self._refs(frame,effect.subject):
                try:obj=self.state.get(ref)
                except RulesViolation:continue
            ...
            for index,(changes,recipients) in enumerate(groups.items()):
                ...
                if isinstance(effect,WhileCounter):
                    ...
                    if isinstance(effect,OngoingEffect):row['duration']='indefinite'
        ...
        elif isinstance(effect,PhaseOut):
            self._phase_out(self._refs(frame,effect.subject))
        ...
        elif isinstance(effect,WinGame):
            if controller in self.state.live_players:
                self.outcome={'kind':'win','winners':[controller],
                    'departed':[p for p in self.state.players if p not in self.state.live_players]}
                self.priority=None;self._event('game_finished',result=self.outcome)
            ...
            events=self._move(tuple(o.ref for o in created),Zone.BATTLEFIELD,frame,key+':tokens',cause='token_created',controller_mode='owner',creates=created,back_face=task.get('token_back_face',False))
            for player in self.turn_order():
            ...
            if isinstance(effect,WithCreatedTokens):self._insert_zone_result(events,Zone.BATTLEFIELD,frame,effect.effects)
    ...
    def _state_based_actions(self):
        ...
        self._combat_prune()
        views=self.characteristics()
        doomed=[];cancellations=[];detaches=[];counter_amounts={};destruction=set();other_deaths=set();saga_deaths=set();protector_choices=[]
        for obj in self.state.objects(Zone.BATTLEFIELD):
            ...
            creature_attachment=bool({'Creature','Battle'}&view.types) and obj.attached_to is not None
            ...
            face_action=self._face_sba(obj,view)
            if face_action=='protector':protector_choices.append(obj)
            if face_action=='sacrifice':saga_deaths.add(obj.ref)
            face_death=face_action in {'permanent_sba','sacrifice'}
            if face_death or zero_loyalty or zero_toughness or illegal_aura:other_deaths.add(obj.ref)
            if lethal_damage:destruction.add(obj.ref)
            if face_death or zero_loyalty or zero_toughness or lethal_damage or illegal_aura:doomed.append(obj)
        legends={}
        for obj in self.state.objects(Zone.BATTLEFIELD):
        ...
        protectors=[]
        for obj in sorted(protector_choices,key=lambda o:players.index(o.controller)):
            legal=tuple(p for p in self.state.live_players if 'Siege' not in views[obj.ref].subtypes or p!=obj.controller)
            choice=self._choose('protector:'+str(self.state.sequence)+':'+self._attachment_key(obj.ref),obj.controller,'battle_protector','Choose a new battle protector.',tuple(Option(p,p) for p in legal),1,1)
            protectors.append((obj.ref,choice[0].key))
        if doomed or cancellations or detaches or protectors:
            source=(doomed[0] if doomed else self.state.get((cancellations or detaches or [protectors[0][0]])[0]))
            self._move(tuple(obj.ref for obj in doomed),Zone.GRAVEYARD,
                ...
                detaches=detaches,counter_pairs=cancellations,destruction_refs=destruction-other_deaths,causes={ref:'sacrifice' for ref in saga_deaths})
            for ref,player in protectors:
                if ref not in {obj.ref for obj in doomed}:
                    self.state.set_protector(ref,player);self._event('battle_protector_changed',source=ref.to_json(),protector=player)
    ...
    def _place_triggers(self):
        ...
        while placement['index']<len(placement['players']):
            ...
            while order:
                ...
                frame['triggered']=True;frame['ability_id']=ability.ability_id;frame['source_must_remain']=ability.source_must_remain.value if ability.source_must_remain else None
    ...
    def advance(self):
        ...
        try:
            while True:
                ...
                if self.phase_action is not None:self._continue_phase_action();continue
