...
function renderLive(s,request,action,board){
 ...
 const health=s.runtime_health||{};const stalled=(health.background_attention||[]).map(x=>`<p role="alert"><strong>${esc(x.actor)} / ${esc(x.role)} needs attention:</strong> ${esc(x.reason)}. Other lanes continue; published guidance may be stale.</p>`).join('');$('#runtimeHealth').innerHTML=stalled+`<p class="muted">${esc(health.scope||'No telemetry available.')}</p><p>Largest input: ${esc(health.largest_input||0)} tokens · ${esc(health.inputs_over_64k||0)} samples above 64k</p><p>Duplicate tool request IDs: ${esc(health.duplicate_request_ids||0)} · rejected tool calls: ${esc(health.rejected_tools||0)}</p>`;
